...
- There are 3,088 records (out of 15,744) where we don't have a heat index value. That limits their value, but we still have temperatures, so we keep them.
...
(We do look at the more strict "Excessive Heat Warning" definition later ...)
...
That said, of the records without heat index values, there are some that do have the `hi_wc_n` value, which we believe to be a "Risk Category" in the NOAA Heat and Humidity Index.
...
```{r}
logs_missing_hi |>
  filter(is.na(hi_wc)) |> # heat index missing
  filter(!is.na(hi_wc_n)) |> # heat index category present
...
> To be clear, the protocol has to be met for three days, then we set the flag the next day.
...
## Peek at Havins unit where end of df shows true values.
logs_protos |> filter(unit == "Havins")
```
...
Here we take our log files and join them with the activation dates so we can see where/when they match.
...
logs_activations_active |> 
  select(unit, date, u_proto_flag, protocol_active)
```
...
Of the 24 unit days under official protocol activation during our time period, only one unit day met our calculations for protocol (Halbert on 7/31/2023). However, 12 of these activation days are within the first two days of our period, meaning they could've met the criteria based on data not in our review.
...
There were 12 "unit days" where strict Excessive Heat Warning conditions were in effect for three or more days during our eight-day period. Official "Incident Command System" protocols were not active on any of those days.
