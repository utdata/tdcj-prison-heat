...
> I rearranged this a little so we could see all years at once. Let's check on the missing units.
...
count_temps_year <- indoor_logs|> 
  ...
  summarize(
    ...
    pct_85 = cnt_85/prison_days,
    pct_90 = cnt_90/prison_days,
    pct_95 = cnt_95/prison_days,
    pct_100 = cnt_100/prison_days
  )  |> 
  mutate(across(starts_with("pct"), round, 2)) # rounds percentage
...
count_temps_unit_year |> 
  ...
  mutate(r_100 = if_else(cnt_100 > 0, "At least 100", "Below 100")) |> 
  filter(year == 2023) |>
  ...
  theme_waffle() +
  labs(
    title = "Number of units to reach 100 in 2023",
    x = "", y = "", fill = "High temp"
  )
```

Now that we know how to do that, we'll do it for multiple years by making a function.
...
Right now I have a variable to filter/set the year. We can make a function later and do all years if we like it.
...
thresh_plot <- thresh_data |> 
  ...
  labs(
    title = paste("Tracking indoor heat in prisons,", set_yr),
    x = "Number of days at each temperature", y = "",
    fill = "Indoor high"
  )

ggsave("figures/thresh_plot.png", width = 6, height = 12)

```

![](figures/thresh_plot.png)
...
```{r}
count_temps_year |> filter(year == 2022) |> select(year, total_days, cnt_90, pct_90)
```
