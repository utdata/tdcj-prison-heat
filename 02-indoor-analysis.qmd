...
indoor_logs |> 
  group_by(year) |> 
  slice_max(temperature, n = 50) |> 
  select(year, unit, date, temperature) 
```

Summarizing the times each unit appeared in the top 50 hottest temperature recordings each year. 
...
> Note for Prof. Mcdonald - this is a new takeaway from the code above that I thought was interesting, but the limitation is some of the recordings are on the same day at different units, which may be close to each other/share the same NWS. Maybe we could filter out repeat days? I also wasn't sure which increments to stick with, so I noted ones where I found interesting temp thresholds. 

Data takeaway: For all complete years of data, the five hottest indoor temperatures recorded each year between April 1 and Sept. 30 across all units all exceeded 103 degrees. The 20 hottest indoor temperatures recorded each year all exceeded 101 degrees. The 50 hottest recordings each year all exceeded 100 degrees, with the exception of five recordings in 2024. 

In 2023, the 15 hottest temperatures recorded all exceeded 103 degrees, while the 20 hottest exceeded 102 degrees. Twenty, or 40% of the 50 hottest temperatures recorded in 2023 were recorded at Garza West, which was the most frequently appearing unit. 
...
## Create a tabset to print out each year 
